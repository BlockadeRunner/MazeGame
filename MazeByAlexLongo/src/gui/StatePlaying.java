...
public class StatePlaying implements State {
    ...
    /**
     * Determines whether game will be played by robot (specified which one) or player.
     */
    String whichRobot;
    
    public void setRobot(String robotType)
    {
    	this.whichRobot = robotType;
    }
    
    /**
     * Robot instantiation for automated play.
     */
    private Robot robot = null;
    
    /**
     * Driver instantiation for automated play.
     */
    ...
    public void start(Control controller, MazePanel panel) {
        ...
        // Check if the game is to be run with a robot and driver
        if(controller.getDriver() != null)
        {
        	// Set the driver to the correct one specified by the controller
        	this.driver = controller.getDriver();
        	
        	// Set the robot to the correct one specified by the controller
        	this.robot = controller.getRobot();
        	
            // set robot controller object to the controller
            this.robot.setController(controller);
            
        	// set robot battery level to its initial setting
            this.robot.setBatteryLevel(3500f);
        	
            // set the driver's robot object to the correct robot
            this.driver.setRobot(this.robot);
            
            // set the driver's maze object to maze
            this.driver.setMaze(this.maze);
            ...
            {
    			// DO NOTHING, will automatically throw null pointer exception when exit is reached.
    		}
            finally 
            {
    			switchFromPlayingToWinning(0, true, this.robot);
    		}
        }
     ...
     */
    public void switchFromPlayingToWinning(int pathLength, boolean robotUsed, Robot robot) {
        ...
        currentState.start(control, panel, robotUsed, robot);
    }
    ...
    public boolean handleUserInput(UserInput userInput, int value) {
        ...
        case UP: // move forward
            ...
            if (isOutside(px,py)) {
                ...
                switchFromPlayingToWinning(0, false, null);
            }
            break;
        case LEFT: // turn left
